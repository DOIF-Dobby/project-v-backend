...
public class ButtonController {
    ...
    @PostMapping
    public ResponseEntity<CommonResponse> insert(@RequestBody @Valid ButtonDto.Insert dto) {
        CommonResponse response = buttonService.insert(dto);
        return ResponseEntity.ok(response);
    }

    @PutMapping("/{id}")
    public ResponseEntity<CommonResponse> update(@PathVariable Long id, @RequestBody @Valid ButtonDto.Update dto) {
