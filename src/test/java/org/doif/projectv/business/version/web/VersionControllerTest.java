...
class VersionControllerTest extends ApiDocumentTest {
    ...
    @Test
    public void 이슈와_맵핑_되지_않은_버전_조회_API_테스트() throws Exception {
        // given
        List<VersionDto.Result> versionResults = new ArrayList<>();
        VersionDto.Result versionResult = new VersionDto.Result(1L, "1.0.1", "버전 설명", 1L, VersionStatus.DEVELOP, "14000", "svn 복사된 태그 정보");
        ...
        given(versionService.searchVersionsNotMappingIssue(eq(1L), any(Pageable.class)))
                .willReturn(pages);
        ...
        // when
        ResultActions result = mockMvc.perform(
                get("/api/versions/not-mapping-issue", 1L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON)
                        .param("issueId", "1")
        );
        ...
        result.andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(response)))
                .andDo(print())
                .andDo(document("version/select-not-mapping-issue",
                        getDocumentRequest(),
                        getDocumentResponse(),
                        requestParameters(
